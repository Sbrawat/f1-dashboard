...
const NextSessionCard = ({ session, loading, error }) => {
    ...
    // Memoize the target date so we aren't recreating the Date object on every render
    ...
    useEffect(() => {
        ...
        const calculateTimeLeft = () => {
            ...
            // Calculate exact time and pad with leading zeros for that telemetry look
            return {
        ...
        // Initial calculation to prevent a 1-second delay flash of '00:00:00:00'
        setTimeLeft(calculateTimeLeft());

        // Update every second, recalculating against the system clock to prevent drift
        const timerInterval = setInterval(() => {
            const newTime = calculateTimeLeft();
            setTimeLeft(newTime);
            
            // If the session went live, kill the interval
            if (newTime.days === '00' && newTime.hours === '00' && newTime.minutes === '00' && newTime.seconds === '00') {
...
const TimeUnit = ({ value, label }) => (
    <div className="flex flex-col items-center bg-gray-950 border border-gray-800 rounded-lg p-3 md:p-4 min-w-[70px] md:min-w-[90px] shadow-inner">
        {/* font-mono gives us tabular figures so the numbers don't jitter when changing width (e.g., 1 vs 0) */}
